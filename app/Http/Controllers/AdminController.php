...
class AdminController extends Controller
{
    ...
    public function addRewardCredits(Request $request){
        ...
        $id = DB::table('invoices')->insertGetId(
            [                                                
                ...
                'payment_ref_number'=>'Free Rewards',
                'created_at'=>$created_at,                
                'invoice_number'=>'Manual_'.$created_at,
                'payer_verify_otp'=>"",
                'otp_expires_on'=>"",                
                'coupon_code'=>'',
            ]
    ...
    public function getStats(Request $request){
        ...
        $processing_files_count = DB::select("SELECT count(upload_id) as total_files_inprogress FROM user_upload WHERE ready_for_download IN('3') "); 

        $total_files_inprogress = $processing_files_count[0]->total_files_inprogress;                     

        
        $data['total_files_inprogress']  = $total_files_inprogress;        
        ...
        if(!$is_admin){
            ...
            $total_inprogress_res = DB::select("SELECT count(upload_id) as total FROM user_upload 
                                        WHERE user_id = $userid AND ready_for_download = '3' ");            
        ...
        /* Revenue Reports */
            ...
            if($is_admin == 1){
                ...
                $revenue_reports_12_weeks = DB::select("SELECT COALESCE(t1.Orders, 0) AS `Orders`, COALESCE(t1.Total, 0) AS `total`, t2.Week AS `Week` FROM ( SELECT COUNT(*) AS `Orders`, ROUND(SUM(amount),0) AS `Total`, DATE(DATE_SUB(created_at, INTERVAL(WEEKDAY(created_at)) DAY)) AS `Week` FROM invoices WHERE 1=1 AND created_at >= NOW() - INTERVAL 12 WEEK GROUP BY 3 ) t1 RIGHT JOIN ( SELECT DATE(DATE_SUB(created_at, INTERVAL(WEEKDAY(created_at)) DAY)) AS `Week` FROM invoices WHERE created_at >= NOW() - INTERVAL 12 WEEK GROUP BY 1 ORDER BY 1 ) t2 USING (Week) ORDER BY week DESC ");
                ...
                array_pop($revenue_reports_12_weeks);    
                for($i=0;$i<(count($revenue_reports_12_weeks));$i++){
                    //$revenue_reports[$i]->revenuedate = date("M-j",strtotime($revenue_reports[$i]->revenuedate));
                    $revenue_reports_12_weeks[$i]->revenuedate = $revenue_reports_12_weeks[$i]->Week;
                }
                ...
                $data['revenue_reports_12_weeks'] = $revenue_reports_12_weeks;
            }
        ...
        $unread=0;
        $mynotification = DB::table('tbl_notifications')
                    ->whereRaw(" FIND_IN_SET('$userid',users) ")
                    ->orderBy("id","desc") 
                    ->first();

        if(!empty($mynotification)){
            $readby = explode(',',$mynotification->read_by);                    
        ...
        $response = ['status'=>'0','msg'=>'','data'=>$data,'unread'=>$unread]; 
        return response()->json($response);
    }
    ...
    public function allowUsersToUpload(Request $request){
        $postData = $request->input();
        $data = [];
        $response = ['status'=>'','msg'=>'','data'=>$data]; 
        extract($postData);

        $access = $status == true ? 'y' : 'n';

        $affected = DB::table('tbl_registration')
              ->where('reg_id',$reg_id)
              ->update(
                        [
                            'upload_allowed' => $access,
                        ]
                    );
        ...
        if(!$orders){
           ...
           // echo "<pre>";print_r($logs);exit;
            ...
            for($i=0;$i<count($orders);$i++){
                ...
                $process_start_time = $orders[$i]->process_start_time;
                $process_end_time = $orders[$i]->process_end_time;                

                if($orders[$i]->ready_for_download == 1){
                    $processing_time = round(((strtotime($process_end_time) - strtotime($process_start_time)) / 60))." minutes";    
                }else{                
                    $processing_time = round(((time() - strtotime($process_start_time)) / 60))." minutes (processing)  ";                          
                }
                
                $orders[$i]->processing_time = $processing_time;
                $orders[$i]->mev_status_ratio  = $orders[$i]->mev_status_ratio.'%';
            }
    ...
    public function getAllUsers(Request $request){        
        ...
        for($i=0;$i<count($users);$i++){            
            ...
            if($users[$i]->upload_allowed == 'y'){
                $users[$i]->upload_allowed = true;
            }else{
                $users[$i]->upload_allowed = false;
            }
