...
class UserController extends Controller
{
    ...
    public function reprocessUnknownEmails(Request $request){
    ...
    public function UploadFileFromAPI(Request $request){
        ...
        if($extension != 'csv' && $extension != 'txt' && $extension != 'xlsx' && $extension != 'xls'){
            ...
            if($extension == 'xls' || $extension == 'xlsx'){
                ...
                $newcsvfilepath = str_replace(".xlsx",".csv",$filestoragepath);
                
                $helper->save($newcsvfilepath,'Csv');
            ...
            if($extension == 'txt'){
                $newcsvfilepath = str_replace(".txt",".csv",$filestoragepath);
                copy($filestoragepath,$newcsvfilepath);
            }          
            ...
            if($no_of_lines > 100){
                ...
                if($total_invoices <= 0){
                    $response = ['status'=>'0','msg'=>'We did not find your purchase details.','data'=>$data,'error_type'=>'credit_limit']; 
                    return response()->json($response);
                    exit;
                }
            }
            ...
            if($userinfo->credits > $total_buys){
                $response = ['status'=>'0','msg'=>'We can not authorize you to validate file as We did not find your purchase details. Please contact support@myemailverifier.com ','data'=>$data];
                return response()->json($response);
                exit;
            }
            ...
            if($mev_robot_active == '0'){  
                
                    if($credits_need > 10000000){

                        $mv_response = Http::attach(
                            'file_contents',file_get_contents($filestoragepath),$newfilename
                        )->post('https://bulkapi.millionverifier.com/bulkapi/v2/upload',['key'=>'IX0MaZHED7hz8uOB034cbVWkI']);
            ...
            $myfileid = DB::table('user_upload')->insertGetId(
                [                      
                    ...
                    'toxic_domains'=>0,               
                    'credit_used'=>0,        
                    'mev_robot_active'=>$mev_robot_active,        
                    'eps'=>$mvfileid,                
                    'status'=>'In Queue',                
                ]
            );
            ...
            if($credits_need <= 10000000 && $mvfileid == ''){

                $textinfo_file_path = storage_path("app/public/uploads/inprocess/$myfileid.txt");  
                $txtfiledetail="$newcsvfilepath \t  $credits_need lines ";
                $textfile = fopen($textinfo_file_path,"w");
                fwrite($textfile,$txtfiledetail);
                fclose($textfile);
            }
    ...
    public function getProcessedFileinfo(Request $request){
        ...
        $unknown_filename=$fileid."_unknown.txt";
        $unknown_csvfilename=$fileid."_unknown.csv";

        $fileInfo = DB::table('user_upload as u')
        ->join('tbl_registration as r', 'r.reg_id', '=', 'u.user_id')
        ->select('u.unknown', 'r.credits')
        ->where('u.upload_id', '=',"$fileid")->first();        

        if($fileInfo->unknown > $fileInfo->credits){            
        ...
        $data = [
                    'available_credits'=>$fileInfo->credits,
                    'credits_need'=>$fileInfo->unknown,
                    'reprocessing_filesname'=>$unknown_filename,
                ];   
        $response = ['status'=>'1','msg'=>'','data'=>$data]; 
    ...
    public function getcsvinfo(Request $request){
        ...
        if($extension != 'csv' && $extension != 'txt' && $extension != 'xlsx'){
            $response = ['status'=>'0','msg'=>'Incorrect File uploaded. Allowed files are txt,csv','data'=>$data,'error_type'=>'invalid_file']; 
        }else{
            ...
            if($extension == 'xls' || $extension == 'xlsx'){
                ...
                //$helper->setUseBOM(false);
                //$helper->setOutputEncoding('SJIS-WIN');
                $helper->save($newcsvfilepath,'Csv');
                $csvfilepath = $newcsvfilepath;            
            }
            ...
            if($no_of_lines > 100){
                $total_invoices = DB::table('invoices')->where('user_id', '=', $userid)->count();        
                if($total_invoices <= 0){
                    $response = ['status'=>'0','msg'=>'Invalid Request','data'=>$data,'error_type'=>'credit_limit']; 
                    return response()->json($response);
                    exit;
                }
            }
            ...
            if($userinfo->credits > $total_buys){
                $response = ['status'=>'0','msg'=>'We can not authorize you to validate file as We did not find your purchase details. Please contact support@myemailverifier.com ','data'=>$data,'error_type'=>'credit_limit'];
                return response()->json($response);
                exit;
            }
    ...
    public function restartUnknownProcess(Request $request){
        $postData = $request->all();
        $current_date = date("Y-m-d H:i");
        $data = [];
        $response = ['status'=>'0','msg'=>'','data'=>$data];
        ...
        $unknownfilename = $fileid."_unknown.txt";
        $newfilename = str_replace(".txt",".csv",$unknownfilename);
        $newfilepath = storage_path("app/public/uploads/$userid/$newfilename");        
        $rel_file_path = "uploads".DIRECTORY_SEPARATOR."$userid".DIRECTORY_SEPARATOR."$newfilename";                

        $unknown_file_path = storage_path("app/public/uploads/inprocess/$unknownfilename");  
        copy($unknown_file_path,$newfilepath);
        unlink($unknown_file_path);

        $credits_need = $uploadsinfo->unknown;        
        ...
        $id = DB::table('user_upload')->insertGetId(
            [                      
                ...
                'valid'=>0,                
                'invalid'=>0,                
                'total'=>$uploadsinfo->unknown,                
                'size'=>$uploadsinfo->unknown,                                
                'unknown'=>0, 
                'spam_trap'=>0,               
                'toxic_domains'=>0,               
                'credit_used'=>0,                                
        ...
        $textinfo_file_path = storage_path("app/public/uploads/inprocess/$id.txt");  
        $txtfiledetail="$newfilepath \t  $credits_need lines ";
        $textfile = fopen($textinfo_file_path,"w");
        fwrite($textfile,$txtfiledetail);
        fclose($textfile);

        $affected2 = DB::update("update  user_upload   
        SET reprocessed = 1 
        WHERE upload_id = $fileid OR upload_id = $id "); 
    ...
    public function startVerifyProcess(Request $request){
        ...
        $myuserinfo = DB::table('tbl_registration')->where('reg_id',$uploadsinfo->user_id)->first();
        $mev_robot_active = DB::table('tbl_admin_settings')->where('id', '1')->value('mev_robot_active');
        $robot_bunch = DB::table('tbl_admin_settings')->where('id', '1')->value('robot_bunch');

        $newfilename=$uploadsinfo->file_name;
        $listname=$uploadsinfo->user_given_name;
        $actual_file_path=$uploadsinfo->file_path;
        $userid=$uploadsinfo->user_id;
        $filesize=$uploadsinfo->size;
        $credits_need=$uploadsinfo->credits_need;
        
        $current_file_path = storage_path("app/public/uploads/$userid/".$uploadsinfo->file_name);  
        $onlyfilename = pathinfo($current_file_path,PATHINFO_FILENAME).".csv";
        $csvfilepath = storage_path("app/public/uploads/$userid/".$onlyfilename);
        ...
        if($filesize <= 10000000 && $mvfileid == ''){

            $textinfo_file_path = storage_path("app/public/uploads/inprocess/$id.txt");  
            $txtfiledetail="$csvfilepath \t  $filesize lines ";
            $textfile = fopen($textinfo_file_path,"w");
            fwrite($textfile,$txtfiledetail);
            fclose($textfile);
        }
    ...
    public function setfilemode(Request $request){               
        ...
        $postData = $request->input();
        $data = [];
        $response = ['status'=>'0','msg'=>'Incorrect req22','data'=>$data]; 
        extract($postData);
        ...
        $file_extension_info = pathinfo($uploadsinfo->file_name);
        $file_extension = $file_extension_info['extension'];
        $current_file_path = storage_path("app/public/uploads/$user_id/".$uploadsinfo->file_name);  
        ...
        //$rolebased_usernames = array('info','support','accounts','admin');        
    ...
    public function stopVerifyProcess(Request $request){
        ...
        $apiresponse = Http::get('https://bulkapi.millionverifier.com/bulkapi/stop/?key=IX0MaZHED7hz8uOB034cbVWkI&file_id='.$uploaded_files_id);

        $apiresponsetext = $apiresponse->body(); 

        echo $apiresponsetext;
    ...
    public function getUserCouponCodes(){
        ...
        $upload_allowed = DB::table('tbl_registration')->where('reg_id', $userid)->value('upload_allowed');

        if(!$coupon_code_list){
            $response = ['status'=>'0','msg'=>'','data'=>[]];
        }else{
            $response = ['status'=>'1','msg'=>'','data'=>$coupon_code_list,'upload_allowed'=>$upload_allowed];    
        }
    ...
    public function validate_single($email='',$apikey=''){
        ...
        $request_allowed=1;
        if($apikey == 'IX0MaZHED7hz8uOB034cbVWkI'){
            if(isset($_SERVER['HTTP_REFERER'])){

                if($_SERVER['HTTP_REFERER'] == 'https://myemailverifier.com/'){
                    $request_allowed=1;
                }else{
                    $request_allowed=0;    
                }
        ...
        if($request_allowed == 0){
            $response = ['status'=>false,'message'=>'Unauthorized Request '];     
            return response()->json($response);
            exit;
        }
        ...
        $credit_balance = ($userdata->credits - 1);

        $today=date("Y-m-d");
        /*$apicall_history = DB::table('tbl_credit_history')
        ->select('*,DATE(created_at) as cdate')
        ->where('user_id', '=',$userid)
        ->where('detail', '=',"API Calls")
        ->where('cdate', '=',"$today")
        ->first();   */

        $apicall_history_obj = DB::select(DB::raw("SELECT *,DATE(created_at) as cdate FROM tbl_credit_history  WHERE user_id = $userid and detail = 'API Calls' AND  DATE(created_at) = '$today' "));     

        //echo "<pre>";print_r($apicall_history_obj);exit;        

        $current_date = date("Y-m-d H:i:s");
        if(empty($apicall_history_obj)){
            ...
            $historyid = DB::table('tbl_credit_history')->insertGetId($historyAr);    

        }else{

            $apicall_history = $apicall_history_obj[0];

            $change = ($apicall_history->credit_change-1);
            ...
            $historyid = DB::table('tbl_credit_history')->where('id',$apicall_history->id)->update($historyAr);  
    ...
    public function resendVerificationCode(Request $request){
            ...
            $invoice_info = DB::table('invoices')->where('invoice_id', '=', $invoice_id)->first();
            
            $userid = $invoice_info->user_id;
            $userInfo = DB::table('tbl_registration')->where('reg_id', '=', $userid)->first();
    ...
    public function creditsPayment(Request $request){
        ...
        if($paypal_email_verified == 'y'){
                ...
                $credit_balance = DB::table('tbl_registration')->where('reg_id', '=',"$userid")->value('credits');        
                $current_date = date('Y-m-d H:i:s');
                $historyAr = [                    
                    'user_id'=>$userid,
        ...
        if($id > 0){
            ...
            $data['invoice_id']=$id;
            //$data['suspicious_order']=$suspicious_order;
    ...
    public function creditHistory(Request $request){        
        ...
        $creditHistory = DB::table('tbl_credit_history')
                ->where('user_id', '=', $userid)
                ->offset($offset)
        ...
        for($i=0;$i<count($creditHistory);$i++){
            $creditHistory[$i]->created_at = date("Y-m-d",strtotime($creditHistory[$i]->created_at));            

            $credits = $creditHistory[$i]->credit_change;

            if($creditHistory[$i]->credit_change <= 0){                
                $creditHistory[$i]->creditadd = false;
                $creditHistory[$i]->credit_change_text="$credits";
            }else{
                $creditHistory[$i]->creditadd = true;
                $creditHistory[$i]->credit_change_text=" + $credits";
            }
        ...
        if(!$creditHistory){
            ...
            $response = ['status'=>'1','msg'=>'','creditHistory'=>$creditHistory];                
        }

        $response['status']=1;
        $response['creditHistory']=$creditHistory;                    
    ...
    public function getValidationRecords(Request $request){
        ...
        if(!$user_uploads){
            ...
            for($i=0;$i<count($user_uploads);$i++){
                ...
                $catchall = $user_uploads[$i]->catchall;
                $spam_trap = $user_uploads[$i]->spam_trap;
                $toxic_domains = $user_uploads[$i]->toxic_domains;
                $total_emails = ($user_uploads[$i]->total - $user_uploads[$i]->blank_lines);    
                $callout_class="callout callout-danger ";
                if($user_uploads[$i]->integration != ''){
                    $integration = " - ".$user_uploads[$i]->integration;
                }                

                /*$estimated_min_time = round((($user_uploads[$i]->size/5)/60)/60,2);
                $estimated_max_time = round((($user_uploads[$i]->size/2)/60)/60,2);   
                $estimated_time = "$estimated_min_time Hours  -  $estimated_max_time Hours";             */
                ...
                if($user_uploads[$i]->ready_for_download > 1){
                    ...
                    if($processed_res > 0 && $total_emails > 0){
                        $progress_percent = round((($processed_res/$total_emails) * 100));    
                    }
                ...
                $valid_csvfile_path = base_path('public/storage/'.$user_uploads[$i]->valid_file_path);
                
                $invalid_file_path = base_path('public/storage/'.$user_uploads[$i]->invalid_file_path);
                $catchall_file_path = base_path('public/storage/'.$user_uploads[$i]->catchall_file_path);
                $unknown_file_path = base_path('public/storage/'.$user_uploads[$i]->unknown_file_path);                
                ...
                if($user_uploads[$i]->ready_for_download == 1){
                    if($user_uploads[$i]->inline_client_app == '1'){
                        if(!file_exists($csvfile_path)){

                            $inline_csv_file_info = pathinfo($user_uploads[$i]->file_path);

                            $inlinecsvfilename=$inline_csv_file_info['basename'];
                            $newclientpath = storage_path("app\public".DIRECTORY_SEPARATOR."uploads".DIRECTORY_SEPARATOR.$userid.DIRECTORY_SEPARATOR."$inlinecsvfilename");  

                            $inliine_files_id = $user_uploads[$i]->upload_id;

                            $mev2fileurl = "https://myemailverifier.com\mev2/public/downloadreport/csv/$inliine_files_id/$inlinecsvfilename";
                            //copy($mev2fileurl,$newclientpath);
                        }    
                        

                        if(!file_exists($xlsfile_path)){

                            $inline_xls_file_info = pathinfo($user_uploads[$i]->xls_file_path);

                            $inlinexlsfilename=$inline_xls_file_info['basename'];
                            $newclientpath = storage_path("app\public".DIRECTORY_SEPARATOR."uploads".DIRECTORY_SEPARATOR.$userid.DIRECTORY_SEPARATOR."$inlinexlsfilename");  

                            $inliine_files_id = $user_uploads[$i]->upload_id;

                            $mev2fileurl = "https://myemailverifier.com/mev2/public/downloadreport/csv/$inliine_files_id/inlinexlsfilename";
                            //copy($mev2fileurl,$newclientpath);
                        }    

                        if(!file_exists($valid_csvfile_path)){

                            $inline_csv_file_info = pathinfo($user_uploads[$i]->valid_file_path);
                ...
                $validcsv_file_url='';
                if(trim($user_uploads[$i]->valid_file_path) != ''){
                    $validcsv_file_url=url('downloadreport/validcsv/'.$user_uploads[$i]->upload_id);
                }

                $invalid_file_url='';
                if(trim($user_uploads[$i]->invalid_file_path) != ''){
                    $invalid_file_url=url('downloadreport/invalidcsv/'.$user_uploads[$i]->upload_id);
                }

                $catchall_file_url='';
                if(trim($user_uploads[$i]->catchall_file_path) != ''){
                    $catchall_file_url=url('downloadreport/catchallcsv/'.$user_uploads[$i]->upload_id);
                }

                $unknown_file_url='';
                if(trim($user_uploads[$i]->unknown_file_path) != ''){
                    $unknown_file_url=url('downloadreport/unknowncsv/'.$user_uploads[$i]->upload_id);
                }
                ...
                $data[] = array(
                    ...
                    'allowstop'=>$allowstop,
                    'reprocessed'=>$user_uploads[$i]->reprocessed,
                    ...
                    'validcsv_file_path'=>$validcsv_file_url,
                    'invalid_file_path'=>$invalid_file_url,
                    'catchall_file_path'=>$catchall_file_url,
                    'unknown_file_path'=>$unknown_file_url,
                    'status'=>$user_uploads[$i]->status,
                    'downloadable'=>$downloadable,
                    'progress'=>'completed',
                    'spam_trap'=>$user_uploads[$i]->spam_trap,
                    'duplicates'=>$user_uploads[$i]->duplicates,
                    'toxic_domains'=>$user_uploads[$i]->toxic_domains,
                    'progress_percent'=>$progress_percent,
                    ...
                    'percent_valid'=>$this->givePercentage($user_uploads[$i]->valid,$total_emails),
                    'percent_invalid'=>$this->givePercentage($user_uploads[$i]->invalid,$total_emails),
                    'percent_unknown'=>$this->givePercentage($user_uploads[$i]->unknown,$total_emails),
                    'percent_catchall'=>$this->givePercentage($user_uploads[$i]->catchall,$total_emails),
                    'percent_spam_trap'=>$this->givePercentage($user_uploads[$i]->spam_trap,$total_emails),
                    'percent_toxic_domains'=>$this->givePercentage($user_uploads[$i]->toxic_domains,$total_emails),
    ...
    public function resendEmailActivationLink(Request $request){
        $postData = $request->input();
        $data = [];
        $response = ['status'=>'','msg'=>'','data'=>$data];
        extract($postData);

        $userInfo = DB::table('tbl_registration')->where('email',$registered_email)->first(); 
        ...
        /* Send Activation link in Email */   
            ...
            $response = ['status'=>'1','msg'=>"Succesfully sent to your email. Please check your spam in case if you don't find them in inbox. ",
                    'data'=>$data];  
    ...
    public function createUser(Request $request){
        ...
        $plussign = strpos($email,"+");

        if($plussign > 0){            
            $response = ['status'=>'0','msg'=>'Invalid Email Address','data'=>$data];          
                return response()->json($response);
                exit;            
        }else{
        ...
        if($emailExists){
            ...
            /* Check Email existance before register */

            //$apiresponse = Http::get('http://23.239.201.239/api/validate_inside.php?apikey=rapid_solo&email='.$email);

            $apiresponse = Http::get('https://api.millionverifier.com/api/v3/?api=IX0MaZHED7hz8uOB034cbVWkI&email='.$email.'&timeout=10');

            $apiresponsetext = $apiresponse->body(); 
            $apiResponseObj = json_decode($apiresponsetext);           
           
            if($apiResponseObj->result != "ok"){
                $response = ['status'=>'0','msg'=>'Invalid Email Address','data'=>$data];          
                return response()->json($response);
                exit;
            }
            ...
            if($id > 1){
                ...
                /* Send Activation link in Email */   
                    ...
                    $response = ['status'=>'1','registered_email'=>$email,'msg'=>'Registered Successfully. Please check your email',
                    'data'=>$data];  
            }else{
    ...
    function email_validation($emailstr) {
        return (!preg_match("^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,3})$^", $emailstr))? FALSE :  TRUE;
    }
